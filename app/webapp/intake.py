...
async def intake_link(
    ...
    if is_media_url(url):
        ...
        kind = 'транскрипт видео' if transcript else 'видео-ссылка'
        return await _analyze_and_store(ctx, user, source_text, kind, 'video', 'video_link')
    ...
    if not text:
        raise HTTPException(400, 'На странице не удалось найти текст для разбора')
...
async def intake_file(
    ...
    if len(data) > max_bytes:
        raise HTTPException(413, f'Файл больше лимита {ctx.settings.max_file_size_mb} МБ')
    if not data:
        raise HTTPException(400, 'Файл пустой')

    if mime.startswith('image/') or suffix in IMAGE_SUFFIXES:
        try:
            result, usage, model, _raw = await ctx.ai.analyze_image(
    ...
    temp_root = Path(ctx.settings.data_dir) / 'tmp' / 'webapp-intake'
        ...
        if mime.startswith('audio/') or suffix in AUDIO_SUFFIXES:
            try:
                async with ctx.stt_sem:
                    transcript = await ctx.stt.transcribe(str(temp_path), 'ru')
            except Exception as exc:
            ...
            if not transcript:
                raise HTTPException(400, 'В аудио не удалось обнаружить речь')
            return await _analyze_and_store(ctx, user, transcript, 'аудио', 'audio', 'audio')

        if suffix in DOCUMENT_SUFFIXES:
            owner = is_creator(user, ctx.settings)
            max_pages = ctx.settings.pro_document_max_pages if (owner or is_active_pro(user)) else ctx.settings.free_document_max_pages
            try:
            ...
            if not extracted:
                raise HTTPException(400, 'В документе не удалось найти текст')
