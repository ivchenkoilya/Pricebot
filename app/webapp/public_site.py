...
def _seller() -> dict[str, str]:
    return {
        key: html.escape(_env(env_name, 'Не заполнено'))
        for key, env_name in {
            'name': 'PUBLIC_SELLER_NAME',
            'inn': 'PUBLIC_SELLER_INN',
            'email': 'PUBLIC_SELLER_EMAIL',
            'support': 'PUBLIC_SUPPORT_CONTACT',
        }.items()
    }
...
def _logo_src() -> str:
    try:
        raw = _LOGO.read_bytes()
    except OSError:
        return '/assets/clarify-banner.webp'
...
async def clarify_logo_v4():
    return await clarify_logo()
